...
class TestRegressionMetrics(unittest.TestCase):
    ...
    def test_MPE_positive_error(self):
        expected_mpe = (
            100 * ((110 - 100) / 100 + (190 - 200) / 200 + (310 - 300) / 300 + (390 - 400) / 400) / 4
        )
    ...
    def test_MPE_zero_in_y_true(self):
        mpe_calculator = MPE(self.y_true_with_zero, self.y_pred_with_zero_adjustment, offset=1e-6)
        result = mpe_calculator.calculate()
        # The exact expected value might need adjustment based on the offset handling
        self.assertNotEqual(0, result)

    def test_MSE_calculation(self):
        expected_mse = np.mean([(110 - 100) ** 2, (190 - 200) ** 2, (310 - 300) ** 2, (390 - 400) ** 2])
    ...
    def test_R2_calculation(self):
        ...
        expected_r2 = 1 - sum((np.array(self.y_true) - np.array(self.y_pred)) ** 2) / sum((np.array(self.y_true) - np.mean(self.y_true)) ** 2)
